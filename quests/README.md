# Quests

Quests sind freiwillige Handlungseinladungen, die Beziehung, Commons, lokale Resilienz oder gemeinsames Handeln stärken.

Quests sind keine Pflichten und kein Ranking-System.

Aktueller Startpunkt:

- [quest-katalog.md](quest-katalog.md) — erste 40 Einladungen für Festival-Kontexte, lokale Kreise und App-v0.1-Nutzerführung.
- [quest-mechanik.md](quest-mechanik.md) — interoperable Quest-Schicht: Autorenschaft, Sichtbarkeit, QuestRun, lokale Completion, Evidence, Confirmations, Badges und Forks.

Sprachlich gilt: Quests sind Teil des Real Life Network Protocol; das Real Life Game ist die spielerische Gestaltungsschicht darüber. Erweiterte Spielmechaniken werden in [real-life-org/real-life-game](https://github.com/real-life-org/real-life-game) beschrieben.

Nächster Schritt:

- App-Card-Texte, Agenten-Trigger und Crew-Fallbacks für die P0-Quests ableiten.
- Erste Quest-Templates für Rollout v0.1 als UI-Karten ableiten.
